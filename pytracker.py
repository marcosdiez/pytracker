...
class Story(object):
    ...
    CSV_FIELDS = ["story_id" , "labels", "story_type", "estimate", "zendesk_id", "owned_by", "name"]
...
class PivotalStatistics(object):
    ...
    def _init_stats(self, owner):
        ...
        self.the_stats[owner]["feature_points"] = 0
    ...
    def _render_stats_helper(self, person, css):
        stringmask = "<tr class='{}''><th>{}<td>{}<td>{}<td>{}<td>{}<td>{}<td>{}\n"
        person_stat = self.the_stats[person]
        total = person_stat["feature_points"] + person_stat["bug"] + person_stat["chore"]
        self.output += stringmask.format(css, person,
            person_stat["zendesk"], person_stat["feature"],
            person_stat["feature_points"], person_stat["bug"], person_stat["chore"], total)
